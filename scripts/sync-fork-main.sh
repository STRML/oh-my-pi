...
# ---------------------------------------------------------------
pr_commits() {
	# shas are from the pre-2026-09-17 layer lineage (85eccd33b0 = last rebuilt layer).
...
47166bcf24 fix(ci): hashline corpus test + fixture guard     (#12170)
...
# conflict-stitched). After the picks, apply the trunk from the recorded layer
# commit 85eccd33b0 (kept forever as provenance). When can1357 merges #12161,
# DELETE trunk_apply and the trunk step.
trunk_apply() {
	git checkout 85eccd33b0 -- \
		...
		packages/coding-agent/src/modes/controllers/setting-side-effects.ts
	python3 scripts/sync-fork-schema-patch.py
	say "pairing trunk applied from layer 85eccd33b0"
}
